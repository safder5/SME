...
class _NewSalesOrderState extends State<NewSalesOrder> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<CustomerProvider>(builder: (context, cP, __) {
      customerNames = cP.getAllCustomerNames();
      suggestions = customerNames;
      return Scaffold(
            ...
            onTap: () async {
              if (soItemsProvider.soItems.isNotEmpty &&
                ...
                try {
                  ...
                  _showMyDialog(context, 'Could not create Sales Order');
                  if (!context.mounted) return;
                  Navigator.pop(context);
                }
              } else if (soItemsProvider.soItems.isEmpty) {
                _showMyDialog(context, 'Avoid empty Orders');
              } else if (nameSearchController.text.isEmpty) {
                _showMyDialog(context, 'Select a Customer or Create New');
              } else {
                _showMyDialog(context, 'Could not create Sales Order');
              }
